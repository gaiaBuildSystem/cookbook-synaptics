...
print("Fetch syna-tools ...", color=Color.WHITE, bg_color=BgColor.GREEN)
...
if not os.path.exists(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-tools"):
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}")
    git clone @(meta["source"]) syna-tools
else:
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-tools")
    git fetch origin

os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-tools")
git checkout @(meta["ref"]["linux/arm64"])

print("Fetch syna-tools, OK", color=Color.WHITE, bg_color=BgColor.GREEN)

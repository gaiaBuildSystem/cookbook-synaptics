...
print("Fetch syna-configs ...", color=Color.WHITE, bg_color=BgColor.GREEN)
...
if not os.path.exists(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-configs"):
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}")
    git clone @(meta["source"]) syna-configs
else:
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-configs")
    git fetch origin

os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-configs")
git checkout @(meta["ref"]["linux/arm64"])

print("Fetch syna-configs, OK", color=Color.WHITE, bg_color=BgColor.GREEN)

...
print("Fetch syna-linux-drivers ...", color=Color.WHITE, bg_color=BgColor.GREEN)
...
if not os.path.exists(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-linux-drivers"):
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}")
    git clone @(meta["source"]) syna-linux-drivers
else:
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-linux-drivers")
    git fetch origin

os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/syna-linux-drivers")
git checkout @(meta["ref"]["linux/arm64"])

print("Fetch syna-linux-drivers, OK", color=Color.WHITE, bg_color=BgColor.GREEN)

...
print("Fetch syna-uboot ...", color=Color.WHITE, bg_color=BgColor.GREEN)
...
if not os.path.exists(f"{_BUILD_PATH}/tmp/{_MACHINE}/su-boot"):
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}")
    git clone @(meta["source"]) su-boot
else:
    os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/su-boot")
    git fetch origin

os.chdir(f"{_BUILD_PATH}/tmp/{_MACHINE}/su-boot")
git checkout @(meta["ref"]["linux/arm64"])

print("Fetch syna-uboot, OK", color=Color.WHITE, bg_color=BgColor.GREEN)

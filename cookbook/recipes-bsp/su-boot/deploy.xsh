...
print("Deploying astra su-boot artifacts ...", color=Color.WHITE, bg_color=BgColor.GREEN)
...
_REPO_PATH = f"{_BUILD_PATH}/tmp/{_MACHINE}/su-boot-usb"
...
if _tool_version_arg:
    sudo \
        ...
        ./gen_x_secure_image \
            ...
            --out_store=@(f"{_DEPLOY_DIR}/uboot_en.usb.bin")
else:
    sudo \
        -E \
        ./gen_x_secure_image \
...
if _chip_name == "dolphin":
    sudo -E ./prepend_image_info.sh \
            @(f"{_DEPLOY_DIR}/uboot_en.usb.bin") \
            @(f"{_DEPLOY_DIR}/gen3_uboot.bin.usb")

    sudo dd if=@(f"{_DEPLOY_DIR}/gen3_uboot.bin.usb") of=@(f"{_DEPLOY_DIR}/gen3_uboot.bin.usb.header") bs=1 count=512


elif _chip_name == "klamath":
    sudo -E cp @(f"{_DEPLOY_DIR}/uboot_en.usb.bin") @(f"{_DEPLOY_DIR}/bootloader_nopreload.subimg")

print("Deploying astra su-boot artifacts, OK", color=Color.WHITE, bg_color=BgColor.GREEN)
